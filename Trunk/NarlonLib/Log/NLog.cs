...
namespace NarlonLib.Log
{
    public static class NLog
    {
        ...
        public static void SetRemote(string ip, int port) //need when use remote log
        {
            remoteIP = ip;
            remotePort = port;
            Start(LogTargets.ServerRemote);
        }

        public static void StartServer(LogTargets target)
        {
            format = "%message"; //logserver only do message record
            Start(target);
        }
        ...
        public static void Start(LogTargets target) //server remote mode will be set auto when use SetRemote
        {
            ...
            if ((type & LogTargets.File) != 0)
            {
                ...
                appender.Layout = new PatternLayout(format);
                appender.File = string.Format("Log/log{0}.txt", DateTime.Now.ToShortDateString());
