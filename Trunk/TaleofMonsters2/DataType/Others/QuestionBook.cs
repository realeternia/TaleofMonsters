...
namespace TaleofMonsters.DataType.Others
{
    internal static class QuestionBook
    {
        ...
        static Question GetQuestionSkillInfo()
        {
            ...
            SkillConfig skl = ConfigData.GetSkillConfig(SkillBook.GetRandSkillId());
            Question question = new Question();
            question.info = string.Format("|技能|Gold|{0}||的{1}是?", skl.Name, cids[type]);
            question.ans = new string[4];
            string attrType = SkillBook.GetAttrByString(skl.Id, eids[type]);
            question.ans[MathTool.GetRandom(4)] = attrType;
            int idx = 0;
            SimpleSet<string> set = new SimpleSet<string>();
            set.Add(SkillBook.GetAttrByString(skl.Id, eids[type]));
            while (idx < 4)
            {
                if (question.ans[idx] != null)
                {
        ...
        static Question GetQuestionInfoSkill()
        {
            ...
            question.ans = new string[4];
            question.ans[MathTool.GetRandom(4)] = skl.Name;
            int idx = 0;
            SimpleSet<string> set = new SimpleSet<string>();
            set.Add(attrType);
            while (idx < 4)
