...
namespace TaleofMonsters.DataType.Others
{
    ...
    {
        public const int SkillCommonCount = 13;
        ...
        public static int[] GetJobLevelAttr(int id, int level)
        {
            ...
            datas[3] = jobConfig.Skl + jobConfig.Sklp*(level - 1)/10;
            datas[4] = jobConfig.Spd + jobConfig.Spdp*(level - 1)/10;
            datas[5] = jobConfig.Luk + jobConfig.Lukp*(level - 1)/10;
            datas[6] = jobConfig.Vit + jobConfig.Vitp*(level - 1)/10;
            datas[7] = jobConfig.Adp + jobConfig.Adpp*(level - 1)/10;

            return datas;
        }

        private static string GetGoodSkill(int id)//获得擅长技能
        {
            List<string> snames = new List<string>();
            for (int i = 0; i < SkillCommonCount; i++)
            {
                if (GetSkillCommonRate(id, i) >= 15)
                {
                    snames.Add(ConfigData.GetHeroSkillCommonConfig((i + 1)*5).Name.Substring(2));
                }
            }
            return string.Join(",", snames.ToArray());
        }
        ...
        public static Image GetPreview(int id)
        {
            ...
            tipData.AddText(string.Format("{0}{1}", stars.Substring(4 - starCount), dstars.Substring(starCount + 1)), "Yellow");
            tipData.AddText("技巧", "White");
            starCount = (jobConfig.Skl * 10 + jobConfig.Sklp) / 20;
            tipData.AddText(string.Format("{0}{1}", stars.Substring(4 - starCount), dstars.Substring(starCount + 1)), "Yellow");
            ...
            starCount = (jobConfig.Vit * 10 + jobConfig.Vitp) / 20;
            tipData.AddText(string.Format("{0}{1}", stars.Substring(4 - starCount), dstars.Substring(starCount + 1)), "Yellow");
